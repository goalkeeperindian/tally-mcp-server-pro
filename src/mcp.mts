...
export async function registerMcpServer(): Promise<McpServer> {
  ...
  mcpServer.registerTool(
    ...
    async (args) => {
      try {
        let result = await queryCollection('Group', ['Name', 'Parent', 'IsRevenue', 'IsDeemedPositive', 'AffectsGrossProfit', 'SortPosition'], new Map<string, string>(), args.targetCompany);
        result = renameObjectArrayProperties(result, new Map<string, string>([['Name', 'group_name'], ['Parent', 'group_parent'], ['IsRevenue', 'bs_pl'], ['IsDeemedPositive', 'dr_cr'], ['AffectsGrossProfit', 'affects_gross_profit'], ['SortPosition', 'sort_position']]));
        let tableID = await cacheTable(new Map<string, string>([['group_name', 'string'], ['group_parent', 'string'], ['bs_pl', 'boolean'], ['dr_cr', 'boolean'], ['affects_gross_profit', 'boolean'], ['sort_position', 'number']]), result);
        return {
  ...
  mcpServer.registerTool(
    ...
    async (args) => {
      try {
        let lstFilters = new Map<string, string>([['Exact_Ledger', `$$IsEqual:$Name:"${args.ledgerName}"`]]);
  ...
  mcpServer.registerTool(
    ...
    async (args) => {
      try {
        let lstFilters = new Map<string, string>([['Exact_StockItem', `$$IsEqual:$Name:"${args.itemName}"`]]);
  ...
  mcpServer.registerTool(
    ...
    async (args) => {
      ...
      let lstLedger = await queryCollection('Ledger', ['Name'], new Map<string, string>([['Exact_Ledger', `$$IsEqual:$Name:"${args.ledgerName}"`]]), args.targetCompany);
  ...
  mcpServer.registerTool(
    ...
    async (args) => {
      ...
      let lstStockItem = await queryCollection('StockItem', ['Name'], new Map<string, string>([['Exact_StockItem', `$$IsEqual:$Name:"${args.itemName}"`]]), args.targetCompany);
